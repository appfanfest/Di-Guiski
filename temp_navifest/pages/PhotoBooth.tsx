...
export const PhotoBooth: React.FC = () => {
  ...
  const currentOverlayUrl = useMemo(() => {
    if (!experience) return "https://i.ibb.co/DD8zRx9G/VACACIONES-03-01.png";
    switch (selectedOverlay) {
      case 2: return experience.demoLink || experience.activationLink;
  ...
  const startCamera = async (retries = 2) => {
    ...
    const constraints: MediaStreamConstraints = retries > 0 
    ...
    try {
      ...
      if (videoRef.current) { 
        ...
        if (isMounted.current) {
            try { await videoRef.current.play(); } catch (playErr) { console.warn("Autoplay was prevented", playErr); }
        }
      }
    } catch (err: any) { 
        if (retries > 0) await startCamera(retries - 1);
        else setCameraError(err.name === 'NotAllowedError' ? "Permiso denegado" : "Error de cámara");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black flex flex-col font-inter overflow-hidden select-none">
      <div className="scanlines opacity-20"></div>
      
      <header className="h-12 border-b border-white/5 flex items-center justify-between px-4 z-[110] bg-black/80 backdrop-blur-md shadow-lg">
        <button 
            onClick={() => navigate(-1)} 
            className="p-1.5 backdrop-blur-md border rounded-lg flex items-center gap-1 font-black text-[9px] uppercase tracking-widest active:scale-90 transition-all shadow-md"
            style={{ 
                color: primaryColor, 
        ...
        <span className="text-white font-orbitron font-black text-[10px] tracking-[0.3em]">
            ATLANTIS <span style={{ color: primaryColor }}>BOOTH</span>
        </span>
      ...
      <div className="flex-1 relative p-1 z-[105] bg-black">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100]">
            {status === 'finished' ? (
                <button 
                  ...
                  className="w-24 h-24 rounded-full backdrop-blur-xl flex flex-col items-center justify-center border-2 active:scale-90 transition-all disabled:opacity-50"
                  style={{
                ...
                >
                    {isExporting ? <Loader2 className="animate-spin" size={30} /> : <Download size={30} />}
                    <span className="text-[7px] font-black mt-1 uppercase tracking-widest">GUARDAR</span>
                </button>
            ) : status === 'idle' ? (
                <button onClick={startSession} disabled={!!cameraError} className="w-24 h-24 rounded-full border-2 border-white/30 bg-white/5 backdrop-blur-md flex flex-col items-center justify-center group active:scale-95 transition-all disabled:opacity-30 shadow-2xl">
                    <div className="w-16 h-16 rounded-full border border-white/40 flex items-center justify-center bg-white/10 group-active:bg-white transition-all">
                        <Camera size={32} className="text-white group-active:text-black" />
                    </div>
                </button>
            ) : null}
        </div>
        ...
        {status === 'countdown' && countdown > 0 && (
          <div className="absolute inset-0 flex items-center justify-center z-[150] pointer-events-none">
            <span className="text-[10rem] font-orbitron font-black text-white/90 drop-shadow-[0_0_50px_rgba(0,0,0,0.8)]">{countdown}</span>
          </div>
        )}

        {status === 'flash' && <div className="absolute inset-0 bg-white z-[200] animate-flash"></div>}
      </div>

      <footer className="h-20 bg-black border-t border-white/5 flex items-center justify-between px-6 z-[110] shadow-2xl">
        <button disabled={status !== 'idle'} onClick={() => setFacingMode(prev => prev === 'user' ? 'environment' : 'user')} className="w-11 h-11 rounded-xl bg-navifest-gray border border-white/10 text-white/50 flex items-center justify-center active:bg-white active:text-black transition-all disabled:opacity-0 shadow-md">
            <RefreshCw size={18} />
        </button>
        ...
        <div className="flex gap-2">
            {[1, 2, 3, 4].map((num) => (
                <button 
                  ...
                  className={`w-9 h-9 rounded-full font-orbitron font-black text-xs transition-all border-2 flex items-center justify-center disabled:opacity-20 shadow-lg ${selectedOverlay === num ? 'text-black border-white scale-110' : 'bg-navifest-gray'}`}
                  style={{
                    backgroundColor: selectedOverlay === num ? secondaryColor : undefined,
                    color: selectedOverlay === num ? 'black' : secondaryColor,
                    borderColor: selectedOverlay === num ? 'white' : `${secondaryColor}30`,
                    boxShadow: selectedOverlay === num ? `0 0 15px ${secondaryColor}50` : 'none'
                  }}
                >
                    {num}
                </button>
        ...
        <button onClick={handleReset} className="w-11 h-11 rounded-xl bg-navifest-gray border border-white/10 text-white/50 flex items-center justify-center active:text-white transition-all shadow-md" style={{ color: '', '--active-bg': primaryColor } as any}>
            <Trash2 size={18} />
        </button>
      </footer>

      {cameraError && (
          <div className="absolute top-16 left-1/2 -translate-x-1/2 z-[200] bg-white text-black px-4 py-1.5 rounded-full font-black text-[9px] uppercase tracking-widest shadow-2xl flex items-center gap-2">
